...
Published = true
[Meta]
Title = "Ransomware in Golang - Part 2"
Description = "Key generation and some scratch encryption..."
---

In case you're late to class, check out parts 0 and 1 at [tacix.at](https://tacix.at).
...
RSA is a cryptosystem taking it's name from three individuals - Rivest, Shamir, and Adleman. There is also a company called RSA Security, and they throw a big conference also called RSA where vendors go to sell things to each other. Here we are talking about the cryptosystem though. 
...
Alright, 程序猿 reporting for duty.
...
First things first, we're importing `crypto/rand`. This is a cryptographically secure random number generator and not a psuedo random number generator. Pseudo random number generators (`math/random`) use some cute math to produce seemingly random numbers which [are often predictable](https://blog.securityevaluators.com/hacking-the-javascript-lottery-80cc437e3b7f). This is fine in cases where it doesnt matter but cryptography is a case where it absolutely does matter.

Next we generate our private key by passing in our source of random bytes and a key size in bits to [`rsa.GenerateKey()`](https://golang.org/pkg/crypto/rsa/#GenerateKey). Here we are using 1024 which is small but [hasn't been factored yet](https://crypto.stackexchange.com/a/1982/80736). I think these can be any size but I've only ever seen multiples of 1024 (i.e. 2048, 3072, 4096). I've seen keys smaller than 1024 that weren't multiples of 1024 but you get the point.

Then we get the public key out of the [private key struct](https://golang.org/pkg/crypto/rsa/#PrivateKey), and make a 16 byte plaintext.

Onto the encryption. We're using `rsa.EncryptOAEP()` from Go's `crypto/rsa`package. Take a look at the [docs](https://golang.org/pkg/crypto/rsa/#EncryptOAEP), get comfortable doing this when writing Go.
...
The first is a hash function, which is used to hash the data gotten from the `random.Reader` that is also passed in. This randomness is used to make sure the same plaintext doesn't encrypt to the same ciphertext. If it did, and say you were encrypting each letter you typed, someone could do a frequency analysis on the ciphertexts and figure out which ones corresponded to which keys.

Next we pass in our a pointer to our public key and the plaintext. The last parameter is a label, if we were encrypting multiple things we might want to label them. That would ensure that one type of encrypted thing couldn't be confused as the other.
...
Wow, look at all that stuff we just learned. `Don't roll your own crypto` is how they keep you **IGNORANT** of the government's **MATH** backdoors. 
...
So the server's private key can decrypt the client's private key, which can in turn unlock the file's encryption keys.
...
Let's get to it then! We'll be generating a keypair, writing our RSA / PKCS1 keys out in an ASN.1 DER encoded format. For this one we're upping the bits to 2k48 because we'll be encrypting the client key (1024 bits) and from our formula `(2048 bits - (32 bytes * 2 * 8 bits/byte) - 2) = 1534 bits`, AKA enough space to fit a 1024 bit key. We could go as low as a 1538 bit server key but using something that isn't a power of 2 would offend the divine numerology and our malware would no longer be blessed.
...
Following that, we generate a key, get the DER encoded bytes of the public key, write them to a file, ditto private key.
...
Now we run it. I'm going to do so in the server directory which doesn't exist yet. Worry if yours outputs the same key as mine.

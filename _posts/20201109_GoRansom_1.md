...
If you check the docs you'll see that the function `Walk()` takes as its second argument is of the type `WalkFunc`. That definition is just below. So you need to pass in a function that matches the `WalkFunc` prototype, `type WalkFunc func(path string, info os.FileInfo, err error) error`. In our case, this is `onVisit()`. In the docs you can check out the definition of the `os.FileInfo` interface, but the important part is that it has an `IsDir()` function that returns true when the visitee is a directory. We can ignore those and return. On files we'll print them out.
